...
class EditDifficultyLevelModal extends Form {
	...
	componentDidMount() {
		...
		const url = `${config.API_URL}/admin/difficulty/${difficultyId}`;
		Axios.get(url, {
			headers: {
	...
	doSubmit = () => {
		...
		const url = `${config.API_URL}/admin/difficulty/${data.id}`;
		Axios.post(url, data, {
			headers: {
				Authorization: Auth.getToken(),
			},
	...
	render() {
		...
		return (
			<Modal show={isEditModalOpen} onHide={onClose} backdrop="static">
