...
class EditPlaylistModal extends Form {
	state = {
		data: {
			name: "",
			thumbnail: "",
	...
	schema = Joi.object({
		...
		thumbnail: Joi.required().label("Thumbnail"),
	});
	...
	doSubmit = () => {
		...
		if(data.thumbnail.length === undefined)
		{
			let playlistData = new FormData();
			playlistData.append("playlistId", this.props.playlistNeedToBeEdit);
			playlistData.append("name", data.name);
			playlistData.append("thumbnail", data.thumbnail);
			Axios.post(`${config.API_URL}/admin/updatePlaylist`, playlistData, {
				headers: {
					...
					if (playlistResponse.status === 200) {
						toast.success("Playlist has been updated successfully.");
	...
	componentDidMount = ()=>{
		...
		Axios.get(`${config.API_URL}/admin/playlist/?playlistId=${this.props.playlistNeedToBeEdit}`, {
			headers: {
				Authorization: Auth.getToken(),
			},
		}).then((response) => {
			this.setState({
				data:{
					...
					thumbnail: [...response.data.data.playlists][0].thumbnail
				},
				userProfilePic: `${config.IMG_URL}/image/playlist/${[...response.data.data.playlists][0].thumbnail}`
			});
		});
		...
		render() {
			...
			return (
				...
				<Modal show={isEditModalOpen} onHide={onClose} backdrop="static" dialogClassName="modal-60w">
